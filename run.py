from flask import Flask, session, render_template, request, redirect, url_for, Response, json, g, abort, flash
...
def checkbuy(stockid,nos,investor,stock):
	...
	if investor.amount_left >= (stock.current_price * nos * 1.01) and stock.shares_left >= nos :
		if stockid == 1 :
			investor.stocks1 += nos
		if stockid == 2 :
			investor.stocks2 += nos
...
def decrease():
	...
	if checksell(stock_id,number_of_stocks,investor):
		investor.amount_left += (stock.current_price * number_of_stocks * 0.985)
		...
		stock.current_price -= 0.5 * number_of_stocks 
...
def increase():
	...
	if checkbuy(stock_id,number_of_stocks,investor,stock):
		investor.amount_left -= (stock.current_price * (number_of_stocks*1.01))
		...
		stock.shares_left -= number_of_stocks
		stock.current_price += 0.6 * number_of_stocks
